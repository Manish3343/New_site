...
class UserDashboardController extends Controller
{
    ...
    public function user_profile_update(Request $request)
    {
        $this->validate($request, [
            ...
            'rl'=>'required',
            'ten_marksheet'=>'required',
            'g_marksheet'=>'required'
        ],[
        ...
        $pan_card=time().'.'.$pan->Extension();
        $request->pan_card->move(public_path('PanCard'), $pan_card);

        $aadhar=$request->aadhar_card;
        $aadhar_card=time().'.'.$aadhar->Extension();
        $request->aadhar_card->move(public_path('AadharCard'), $aadhar_card);

        $rl=$request->rl;
        $rl_card=time().'.'.$rl->Extension();
        $request->rl->move(public_path('RLCard'), $rl_card);

        $ten=$request->ten_marksheet;
        $ten_marksheet=time().'.'.$ten->Extension();
        $request->ten_marksheet->move(public_path('TenMarksheet'), $ten_marksheet);

        $gray=$request->g_marksheet;
        $g_marksheet=time().'.'.$gray->Extension();
        $request->g_marksheet->move(public_path('GraduationMarksheet'), $g_marksheet);
        ...
        $ids=Auth::user()->id;
        Client::where('user_id',$ids)->update([
           'name'=>$request->name,
           'email'=>$request->email,
           'pan_card'=>$pan_card,
           'aadhar_card'=>$aadhar_card,
           'rl_card'=>$rl_card,
           'tenth_mark'=>$ten_marksheet,
           'g_mark'=>$g_marksheet
        ]);
